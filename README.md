...
> JADX 反编译产物(`bili/sources`,约 35,038 个 `.java`)。本文由 20 个并行分析 Agent + 人工交叉验证汇总。
> 一句话定位:这棵树里能看到的是**直播在主壳工程里的部分**——开播推流 SDK、看播播放器解析/状态、对宿主的服务门面、以及搜索/番剧/课程/详情页等场景化接入;**完整的"直播间" `com.bilibili.bililive.room`(送礼/弹幕/PK/连麦 UI)是独立插件,本次未全量解出**(与 `com.mall` 会员购模块情况一致)。
